...
sys.path.append(r'C:\Users\Yatharth\sort')  # Replace with the actual path to the SORT directory
...
while cap.isOpened():
    ...
    if all_faces:
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                (x, y, w, h) = all_faces[i]
                detections.append([x, y, x + w, y + h, all_scores[i]])

    # Update tracker with current detections if any
    if len(detections) > 0:
    ...
    # Display the frame
    cv2.imshow('Ensembled Face Detection and Tracking', frame)
